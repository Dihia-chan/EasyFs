...
function formatSrc($src,$clid) {
	...
	if (empty($src)) {
		echo '<td class="record_col">Unknown</td>' . PHP_EOL;
	} else {
...
function formatApp($app, $lastdata) {
	echo '<td class="record_col"><abbr class="simptip-position-top simptip-smooth simptip-fade" data-tooltip="Application: '.$app.'('.$lastdata.')">'.$app.'</abbr></td>' . PHP_EOL;
}
...
function formatDst($dst, $dcontext) {
	...
	if (is_numeric($dst) && strlen($dst) >= $rev_min_number_len  && strlen($rev_lookup_url) > 0) {
		...
		echo '<td class="record_col"><abbr class="simptip-position-top simptip-smooth simptip-fade" data-tooltip="Contexte de destination: '.$dcontext.'"><a href="'.$rev.'" target="reverse">'.$dst.'</a></abbr></td>' . PHP_EOL;
	} else {
		echo '<td class="record_col"><abbr class="simptip-position-top simptip-smooth simptip-fade" data-tooltip="Contexte de destination: '.$dcontext.'">'.$dst.'</abbr></td>' . PHP_EOL;
	}
}
...
function formatDisposition($disposition, $amaflags) {
	...
	switch ($disposition) {
		...
		case 'FAILED':
			...
			$dispTxt = 'Pas de r&eacute;action';
			$img = 'img/status_failed.png';
			$style = 'failed';
			break;
		case 'NO_ROUTE_DESTINATION':
			$dispTxt = 'pas d&acute;itin&eacute;raire';
			$img = 'img/status_failed.png';
			$style = 'failed';
			break;	
		case 'INVALID_GATEWAY':
			$dispTxt = 'extension invalide';
			$img = 'img/status_failed.png';
			$style = 'failed';
			break;		
		default:
...
function formatDuration($duration, $billsec) {
	...
	echo '<td class="record_col"><abbr class="simptip-position-top simptip-smooth simptip-fade" data-tooltip="En facturant: '.$billduration.'">'.$duration.'</abbr></td>' . PHP_EOL;
}
...
function callrates($dst,$duration,$file) {
	...
	if (!array_key_exists($file, $callrate_cache)) {
		...
		$fr = fopen($file, 'r') or exit('Impossible d&acute;ouvrir le fichier avec les taux ('.$file.')');
