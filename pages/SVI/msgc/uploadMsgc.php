...
rename($_FILES["monfichier"]["name"], "/usr/local/freeswitch/sounds/en/us/callie/svi/48000/".$_FILES["monfichier"]["name"]);
    ...
    $doc = new DomDocument();
            ...
            // <--! creer elt user ->
            $macro=$doc->createElement('macro');
            $macro->setAttribute('name', $_POST['id']);
            $macro->setAttribute('pause', '100');
            $include->appendChild($macro); 
            // params
            ...
            $action->setAttribute('function', 'play-file' );
            $action->setAttribute('data', 'svi/'.$_FILES["monfichier"]["name"] );   
            $match->appendChild($action);

            $doc->save("/usr/local/freeswitch/conf/lang/en/demo/".$_POST['id'].".xml");
            ...
            $id = htmlspecialchars($_POST['id']);
             ...
             $requete="INSERT INTO `macroc`(`date`, `macroc`, `enrg`, `descp`) VALUES ('".strftime("%F %T")."','".$_POST['id']."','".$_FILES["monfichier"]["name"]."','".$_POST['descp']."')" ;
